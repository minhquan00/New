            ...
            tlsSocket = tls.connect({
                    ...
                    while (data.length >= 9) {
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 7 || frame.type == 5) {
                                ...
                                //Xử lý RST_STREAM (kiểu 5)
                                if (frame.type == 5) {
                                    // Gửi luồng mới ngay lập tức
                                    streamId += 2;
                                    continue;
                                }
                            }
                ...
                function doWrite() {
                    ...
                    if (randrate !== undefined) {
                        ratelimit = getRandomInt(1, 200); // Tăng range để tăng RPS
                    } else {
                        ratelimit = process.argv[6];
                    }
                    
                    // TĂNG SỐ REQUEST PER BATCH cho HIGH RPS
                    const batchSize = isFull ? ratelimit : Math.min(ratelimit, 10);
                    ...
                    tlsSocket.write(Buffer.concat(requests), (err) => {
                        if (!err) {
                            ...
                            const nextDelay = isFull ? 80 : Math.max(50, 100 / ratelimit);
                            setTimeout(() => {
                                doWrite()
                            }, nextDelay)
                        }
